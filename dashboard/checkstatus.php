...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Retrieve complaint ID from the form
    $complaint_id = $_POST['complaint_id'];

    // SQL query to retrieve the status of the complaint
    $sql = "SELECT status FROM complaints WHERE id = $complaint_id";
    $result = $conn->query($sql);

    if ($result->num_rows > 0) {
        // Complaint found, retrieve the status
...
<div class="row">
    <h2 class="mt-4 mb-4">Check Complaint Status</h2>
    <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
        ...
        <button type="submit" class="btn btn-primary mt-5">Check Status</button>
    </form>
    <div class="mt-4">
        <h4>Status:</h4>
        <?php echo $status; ?>
